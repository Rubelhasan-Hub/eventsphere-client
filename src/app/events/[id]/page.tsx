...
async function getEventDetails(id: string): Promise<IEvent | null> {
  ...
  const event = await db.collection<IEvent>("events").findOne({ _id: new ObjectId(id) });
  client.close();
  ...
  return (
    <main className="max-w-5xl mx-auto px-4 py-16">
      <div className="grid md:grid-cols-2 gap-12">
        <img src={event.imageUrl} alt={event.title} className="w-full h-96 object-cover rounded-3xl" />
        <div className="space-y-6">
          ...
          <button className="bg-indigo-600 text-white px-8 py-3 rounded-xl font-bold">Book Now</button>
        </div>
      </div>
    </main>
  );
